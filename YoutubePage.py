...
class youtube_page:
    ...
    def __init__(self, root, parent, bg_img):
        ...
        self.file_label_border = Frame(
            file_browser_frame, highlightbackground=resources.button_color, highlightthickness=3, bd=0, background="black", highlightcolor=resources.button_color)
        ...
        self.file_title.element = self.youtube_canvas.create_text(self.center_x_loc - self.file_title.x_offset, self.center_y_loc - self.file_title.y_offset, anchor=W,
                                                                 text="Song Youtube URL", fill="white", font=(self.font_name, 14, self.font_weight))
        ...
        file_save_frame = Frame(self.youtube_canvas, background="#000000", bd=0)
        ...
        self.save_browser_button.element = self.youtube_canvas.create_image(self.center_x_loc,
                                                                           self.center_y_loc, tags="saveButton", image=self.save_button_img, anchor="center")
        self.youtube_canvas.tag_bind(
            "saveButton", "<Button-1>", self.browse_save_location)
        self.youtube_canvas.tag_bind(
            "saveButton", "<Enter>", lambda event: on_cursor_overlap(self.youtube_canvas))
        self.youtube_canvas.tag_bind(
            "saveButton", "<Leave>", lambda event: on_cursor_endoverlap(self.youtube_canvas))
        ...
        # Split Button
        # --------------------------------------------------------------------------
        split_glob_off = -76  # increase to move down

        self.split_button = canvas_element()
        self.split_button.x_offset = 0
        self.split_button.y_offset = split_glob_off
        self.split_button.element = self.youtube_canvas.create_image(self.center_x_loc,
        ...
        self.youtube_canvas.tag_bind(
            "splitButton", "<Enter>", lambda event: on_cursor_overlap(self.youtube_canvas))
        self.youtube_canvas.tag_bind(
            "splitButton", "<Leave>", lambda event: on_cursor_endoverlap(self.youtube_canvas))
        # --------------------------------------------------------------------------
        ...
        self.output_border = Frame(self.youtube_canvas, highlightbackground="#b096ff", highlightcolor="#b096ff", highlightthickness=2, bd=0, background="black")
        self.output_label = Text(self.output_border, bg="black", fg="white", width=49, height=15, font=(self.font_name, 10, self.font_weight), bd=0)
        self.output_label.insert("end", "Welcome to the Youtube Mp3 Downloader!\nWaiting for input...")
    ...
    def resize_handler(self, event):
            ...
            # Splits Button
            self.youtube_canvas.coords(self.split_button.element, self.center_x_loc -
            ...
            # self.youtube_canvas.coords(self.youtube_text, self.center_x_loc, self.center_y_loc - 20)
